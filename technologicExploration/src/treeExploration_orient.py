...
def assignGoals(curr_root,actual_successors,robotsList,occupancyGridMatFin):
    ...
    if(succ):
        ...
        assigned_node_index=1
        for assigned_node in assigned_nodes:
            if(assigned_node==curr_root):
                robot="robot"
                robot+=str(assigned_node_index)
                topicName="/"
                topicName+=robot
                topicName+="/move_base"
...
def checkStatus():
    ...
    global aborted_list
    global interim_goals_list
    global G
    counter=1
    if(not goal_reached.empty()):
        curr_goal=goal_reached.get()
        msg = rospy.wait_for_message("/projected_map", OccupancyGrid)
        index_robot=curr_goal[1]
        current_status=curr_goal[0]
        ...
        for robot in list_of_robots:
            curr_feedback_topic="/"
            curr_feedback_topic+=robot
            curr_feedback_topic+="/orienteering_feedback"
            curr_feedback = rospy.wait_for_message(curr_feedback_topic, String)
            if(curr_feedback.data=="Goal Reached"):
                current_status=cluster_status[index_robot]
                goal_to_queue=[]
                goal_to_queue.append(current_status)
                goal_to_queue.append(index_robot)
                goal_to_queue.append(list_of_robots[index_robot:index_robot+1])
                goal_reached.put(goal_to_queue)
            index_robot+=1
    rospy.sleep(2)
    checkStatus()

def checkStatus_orient():
    rospy.loginfo("Checking status!")
    global goal_reached
    global move_base_list
    global list_of_robots
    ...
    if(not goal_reached.empty()):
        curr_goal=goal_reached.get()
        send_multiple_goals(curr_goal[0],curr_goal[1],curr_goal[2])
    else:
        if(move_base_list):
            for move_base in move_base_list:
                index_robot=counter-1
                if(interim_goals_list[index_robot] and len(interim_goals_list[index_robot])==1):
                    #move_base=goal_list[0]
                    goal_states = ['PENDING', 'ACTIVE', 'PREEMPTED', 'SUCCEEDED', 'ABORTED', 'REJECTED','PREEMPTING', 'RECALLING', 'RECALLED','LOST']
                    failed_states=['ABORTED', 'REJECTED','LOST','RECALLED']
                    ...
                    next_root=assigned_nodes[index_robot]
                    state = move_base.get_state()
                    print("THIS IS THE CURRENT STATE (FINAL GOAL):")
                    print(state)
                    print("robot number")
                    print(index_robot)
                    ...
                    if (state == GoalStatus.SUCCEEDED and StatusChecker[index_robot]>2):
                        ...
                        if(current_status==8):
                            for num in range(0,8):
                                while(len(interim_goals_list[num])>0):
                                    interim_goals_list[num].pop()
                            callback(msg,next_root,list_of_robots[:])
                        elif(current_status==4):
                            if(index_robot in (0,1,2,3)):
                            ...
                            interim_goals_list[index_robot].pop()
                            callback(msg,next_root,list_of_robots[index_robot:index_robot+1])
                ...
                elif(interim_goals_list[index_robot] and len(interim_goals_list[index_robot])>1):
                    #move_base=goal_list[0]
                    goal_states = ['PENDING', 'ACTIVE', 'PREEMPTED', 'SUCCEEDED', 'ABORTED', 'REJECTED','PREEMPTING', 'RECALLING', 'RECALLED','LOST']
                    failed_states=['ABORTED', 'REJECTED','LOST','RECALLED']
                    move_base.wait_for_server(rospy.Duration(60))
                    ...
                    next_root=assigned_nodes[index_robot]
                    state = move_base.get_state()
                    print("THIS IS THE CURRENT STATE (INTERIM GOAL) :")
                    print(state)
                    print("robot number")
                    print(index_robot)
                    current_status=cluster_status[move_base_list.index(move_base)]
                    if ((state == GoalStatus.SUCCEEDED or state == GoalStatus.ABORTED) and StatusChecker[index_robot]>2):
                        StatusChecker[index_robot]=0
                        print("reached interim goal point : going to next point")
                        interim_goals_list[index_robot].pop(0)
                        goal_to_queue=[]
                        goal_to_queue.append(current_status)
                        goal_to_queue.append(index_robot)
                        goal_to_queue.append(list_of_robots[index_robot:index_robot+1])
                        goal_reached.put(goal_to_queue)
                        #send_multiple_goals(current_status,index_robot,list_of_robots[index_robot:index_robot+1])
                    else:
                        StatusChecker[index_robot]+=1
                counter+=1
    rospy.sleep(2)
...
def send_multiple_goals(current_status,index_robot,listOfRobots):
    goal_states = ['PENDING', 'ACTIVE', 'PREEMPTED', 
                       'SUCCEEDED', 'ABORTED', 'REJECTED',
                       'PREEMPTING', 'RECALLING', 'RECALLED',
                       'LOST']
    move_base=move_base_list[index_robot]
    rospy.loginfo("Waiting for move_base action server...")
    move_base.wait_for_server(rospy.Duration(300))
    rospy.loginfo("Connected to move base server")
    rospy.loginfo("Starting Exploration")
    goal_list = interim_goals_list[index_robot]
    goal=goal_list[0]
...
def publishGraph(s,t,robotName):
    epsilon = 1
    topicName="/"
    topicName+=robotName
    topicName+="/move_base/NavfnROS/make_plan"
    ...
    return_graph=PoseArray()
    return_graph.header.frame_id="map"
    return_graph.header.stamp = rospy.Time.now()
    FinalPublisher = rospy.Publisher("/orienteering_goal", PoseStamped)
    if(poses):
        processed_grid_points=[]
        prev_pose=PoseStamped()
        prev_pose=poses[0]
        topic = '/graph_marker_array'
        print("==================================")
        print(len(poses))

        graph_delta=round(len(poses)/20)

        GraphPublisher = rospy.Publisher(topic, MarkerArray)
        graphArray = MarkerArray()
        #graphArray.header.frame_id="map"
        #graphArray.header.stamp = rospy.Time.now()
        row_counter=1
        for poseStamped in poses:
            gol1=np.array((poseStamped.pose.position.x,poseStamped.pose.position.y))
            gol2=np.array((prev_pose.pose.position.x,prev_pose.pose.position.y))
            dist=np.linalg.norm(gol1-gol2)
            if(dist>=max(graph_delta,0.5) and poses.index(poseStamped)<(len(poses)-5)):
                prev_pose=poseStamped
                #Create the grid around path
                x1=poses[poses.index(poseStamped)-5].pose.position.x
                y1=poses[poses.index(poseStamped)-5].pose.position.y
                x2=poses[poses.index(poseStamped)+5].pose.position.x
                y2=poses[poses.index(poseStamped)+5].pose.position.y

                xp=poseStamped.pose.position.x
                yp=poseStamped.pose.position.y
                #Get slope of line
                if(x2!=x1 and y2!=y1):
                    m1=(y2-y1)/(x2-x1)
                ...
                while(iter_x<=xp+1):
                    iter_y=yp-1
                    while(iter_y<=yp+1):
                        if((iter_y-yp-(m2*iter_x)+(m2*xp)<0.5) and (iter_x==xp or iter_y==yp)):#and (iter_x!=xp and iter_y!=yp)):
                                    marker = Marker()
                                    marker.header.frame_id = "/map"
                                    ...
                                    marker.pose.position.z=0
                                    graph_pose=copy.copy(marker.pose)
                                    graph_pose.position.z=row_counter
                                    point_list=(iter_x,iter_y)
                                    pi,pj=getindices(point_list)
                                    valid_graph_pose=True
                                    if(pi>2 and pj>2 and pi<height-2 and pj<width-2):
                                        for added_poses in return_graph.poses:
                                            added=np.array((added_poses.position.x,added_poses.position.y))
                                            current=np.array((graph_pose.position.x,graph_pose.position.y))
                                            dist_add_curr=np.linalg.norm(added-current)
                                            #print(dist_add_curr)
                                            #rospy.sleep(1.0)
                                            if(dist_add_curr<0.25):
                                                valid_graph_pose=False
                                        if(valid_graph_pose and checkReachability(marker.pose,robotName)):
                                            return_graph.poses.append(graph_pose)
                                            graphArray.markers.append(marker)
                                    id=0
                                    for m in graphArray.markers:
                                        m.id = id
                                        id += 1
                        iter_y+=0.5
                    iter_x+=0.5
        ...
        rospy.sleep(0.1)
        FinalPublisher.publish(goal)
        GraphPublisher.publish(graphArray)
    else:
        print("NADA")
    return return_graph

def checkReachability(graph_pose,robotName):
    topicName="/"
    topicName+=robotName
    topicName+="/move_base/NavfnROS/make_plan"
    ...
    start = PoseStamped()
    start.header.frame_id = 'map'
    start.pose=get_pose(robotName)

    goal = PoseStamped()
    goal.header.frame_id = 'map'
    goal.pose=graph_pose
    tolerance=0.0
    move_base_plan = rospy.ServiceProxy(topicName, GetPlan)
    plan_ret = move_base_plan(start,goal,tolerance)
    poses=plan_ret.plan.poses
...
def sendGoalRectilinear(next_root,tup_goal,robotName,occupancyGridMatFin):
    global G
    global move_base_list
    #global orienteering budget
    print("in goal")
    print(tup_goal)
    print(robotName)

    s=get_pose(robotName)
    t=Pose()
    t.position.x=tup_goal[0]
    t.position.y=tup_goal[1]
    t.orientation.w=1
    input_graph = PoseArray()
    input_graph=publishGraph(s,t,robotName)
    gol1=np.array((t.position.x,t.position.y))
    gol2=np.array((s.position.x,s.position.y))
    dist_gol_to_root=np.linalg.norm(gol1-gol2)
    orienteering_path = PoseArray()

    # test publisher
    topicName="/"
    topicName+=robotName
    topicName+="/orienteering"
    orienteering_path_publisher = rospy.Publisher(topicName, PoseArray)
    print("calling service to get orienteering path")
    try:
        get_orienteering_path = rospy.ServiceProxy('/get_orienteering_path',orienteering)
        resp1 = get_orienteering_path(s,t,dist_gol_to_root*2,robotName,input_graph)
        orienteering_path=resp1.orienteering_path
    except rospy.ServiceException, e:
        print "Service call failed: %s"%e

    #test publisher
    orienteering_path_publisher.publish(orienteering_path)

    # list_of_goals=[]
    # counter=0
    # index_robot = int(robotName[-1])-1
    # for point in orienteering_path.poses:
    #     goal_states = ['PENDING', 'ACTIVE', 'PREEMPTED', 
    #                    'SUCCEEDED', 'ABORTED', 'REJECTED',
    #                    'PREEMPTING', 'RECALLING', 'RECALLED',
    #                    'LOST']
    ...
    #         rospy.loginfo("Starting Exploration")
    #     goal = MoveBaseGoal()
    #     goal.target_pose.pose.position.x = point.position.x
    #     goal.target_pose.pose.position.y = point.position.y
    ...
    # print(index_robot)
    # print(robotName)
    # print(len(interim_goals_list))
    # interim_goals_list[index_robot]=(list_of_goals)



def sendGoalRectilinear_old(next_root,tup_goal,robotName,occupancyGridMatFin):
    global G
    global move_base_list
    print("in goal")
    print(tup_goal)
...
def scan(curr_root,no_of_robs):
    ...
    interim_goals_list=[None] * total_no_of_robots
